...
def train_stgcn():
    ...
    print(f"[*] Training ST-GCN on device: {device}")
    ...
    BATCH_SIZE = 32
    EPOCHS = 2  # Đặt là 2 chỉ để test nhanh, hãy đổi thành 50-100 khi có data thật
    LEARNING_RATE = 1e-3
    TARGET_FRAMES = 60
    NUM_CLASSES = 100
    
    # Quan trọng: Đặt return_graph=True để dữ liệu giữ nguyên cấu trúc không gian 3 chiều
    ...
    best_model_path = os.path.join(script_dir, "weights", "best_stgcn_model.pth")
    
    history = {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': []}
    
    print("[*] Starting ST-GCN training loop...")
    ...
    history_path = os.path.join(script_dir, "weights", "history_stgcn.json")
    with open(history_path, 'w', encoding='utf-8') as f:
        json.dump(history, f, indent=4)
    print(f"[*] Saved ST-GCN training history to: {history_path}")
